...
test("ignores bot messages", async () => {
    ...
    await handleGeminiSummaryMessage(message, createDependencies({
...
test("summarizes the previous 24 hours when a human sends the trigger", async () => {
    ...
    expect(getReplyContent((message as MessageFixture & { replies: ReplyPayload[] }).replies[0])).toBe("Resumo conciso do chat");
});
...
test("replies with a friendly error when the summary provider fails", async () => {
    ...
    expect(getReplyContent((message as MessageFixture & { replies: ReplyPayload[] }).replies[0])).toMatch(/nao consegui|erro|falha/i);
});
